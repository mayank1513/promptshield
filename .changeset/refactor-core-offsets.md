...
- Introduced `enrichWithRange` utility for post-processing character offsets into human-readable locations.
- Simplified `scan` entry point by removing manual line-splitting logic.
- Updated `ThreatReport` to use a `range` object.
- Added `ThreatReportWithRange` for diagnostics-ready reports.
